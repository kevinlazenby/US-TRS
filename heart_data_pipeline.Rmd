...
if (RData_file_loaded == FALSE) {
  ...
  save(cand_thor, column_descriptions, just_form_hr, just_form_hr_data_link,
       ...
       file = here(curr_data_release, raw_data_filename))
} else {
  start_time <- Sys.time()
...
filter_date_end <- mdy("12-31-2022")
...
cand_list <- cand_thor %>% 
  ...
  # Remove recipients transplanted after December 31, 2022 (insufficient follow-up)
...
tx_list <- tx_list %>% 
  ...
  select(PX_ID, ORG_TY, REC_TX_DT, REC_POSTX_LOS, REC_A_MM_EQUIV_TX,
         ...
         REC_ACUTE_REJ_EPISODE, REC_CTR_CD, REC_AGE_IN_MONTHS_AT_TX,
...
if (include_pre_policy == TRUE) {
  ...
  just_stat1a_1b <- just_stat1a_1b %>%
    ...
    select(RowNumber, PX_ID, CANHX_STAT_CD, 
           ...
           CAN_LISTING_CTR_CD, CANHX_CHG_DT, CANHX_VAD_IMPLANT_DT, CANHX_LVAD_DT,
...
full_list <- full_list %>% 
  ...
  # fill variables "down" to show change over time
  # this fills in missing values using the na.locf() method
  # must uncomment 5 lines below if using pre-policy data
  ...
  # must uncomment 1 line below if using pre-policy data
...
save(final_list, file = here(curr_data_release, rdata_filename))
write_csv(final_list, file = here(curr_data_release, csv_filename))
